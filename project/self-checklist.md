...
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        |         +         |`src/service/` - скачивание обученной модели, `src/models/`- обучение выбранной модели            |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          |         +         |`notebooks/01_eda.ipynb`, `report.md`|
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        |         +         |`notebooks/02_baselines.ipynb`, `report.md` |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`            |       +           |`src/data/`, `src/models/`, `src/service/`, `src/utils/` |
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker       |         +         |`dockerfile` или шаги в `README.md`       |
| 7  | Есть `.env.example` и **нет** в репозитории реальных секретов/паролей    |       +           | `.env.example`, отсутствие `.env`, отсутствие секретов в проекте как таковых    |
| 8  | Реализованы логи/наблюдаемость (консольные логи + файл logs.log + `/health`)     |        +          |Использование `logging` (project\src\utils\logging.py), endpoint `/health` |
